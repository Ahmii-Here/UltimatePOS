$(document).ready(function() {
    ...
    if(yearFormat == "BS"){
        ...
        if ($('#expense_date_range').length == 1) {
            $('#expense_date_range').daterangepicker(
                dateRangeSettings, 
                function(start, end) {
                    // Convert selected Nepali BS dates to AD dates for AJAX request
                    var adStartDate = convertToAD(start.toDate());
                    var adEndDate = convertToAD(end.toDate());
        
                    // Update the value of the date range picker to show Nepali dates
                    ...
                    // Now set the global variables or however you manage state
                    globalADStartDate = adStartDate;
                    globalADEndDate = adEndDate;
        
                    // Reload the expense table after setting the global variables
        ...
        var globalADStartDate = convertToAD($('#expense_date_range').data('daterangepicker').startDate.toDate());
        var globalADEndDate = convertToAD($('#expense_date_range').data('daterangepicker').endDate.toDate());
    ...
    expense_table = $('#expense_table').DataTable({
        ...
        ajax: {
            url: '/expenses',
            data: function(d) {
                d.start_date = moment(globalADStartDate).format('YYYY-MM-DD');
                d.end_date = moment(globalADEndDate).format('YYYY-MM-DD');
...
function updateProfitLoss(start = null, end = null, location_id = null, selector = null) {
    ...
    $.ajax({
        ...
        success: function(html) {
            ...
            updateStockBySellingPrice(data); // Ensure this function exists and is correctly implemented
        },
    });
