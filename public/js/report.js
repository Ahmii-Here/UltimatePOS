$(document).ready(function() {
    ...
    // Check if the date range picker exists
    if ($('#scr_date_filter').length == 1) {
        $('#scr_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
            // Include BS to AD conversion based on a condition, e.g., a global variable or another input's value
    ...
    //contact report
   var supplier_report_tbl = $('#supplier_report_tbl').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            ...
            data: function(d) {
                ...
                var dateRange = $('input#scr_date_filter').val().split(' ~ ');
                if (dateRange.length === 2) {
                    var start = dateRange[0];
                    var end = dateRange[1];
                    var yearFormat = $('#year-format').val(); // Assuming you have a way to select the year format
                    if (yearFormat === 'BS') {
                        start = convertToAD(start);
                        end = convertToAD(end);
                    }
                    d.start_date = start;
                    d.end_date = end;
        ...
        if ($('#sr_payments_with_commission_table').length > 0) {
            ...
            $('table#sr_payments_with_commission_table').DataTable({
                ...
                ajax: {
                    ...
                    data: function(d) {
                        var yearFormat = $('#year-format').val(); // Example: Determine the year format
            
                        // Assuming daterangepicker is attached to #sr_date_filter
                        var start = $('#sr_date_filter').data('daterangepicker').startDate.format('YYYY-MM-DD');
                        var end = $('#sr_date_filter').data('daterangepicker').endDate.format('YYYY-MM-DD');
            
                        // Directly apply conversion within the AJAX data function if format is BS
                        if (yearFormat === 'BS') {
                            // Direct conversion logic for BS to AD (example conversion, adjust as necessary)
                            const bsToADYears = 56;
                            const bsToADMonths = 8;
                            const bsToADDays = 17; // Adjusted for reverse conversion accuracy
                            start = moment(start, 'YYYY-MM-DD').subtract(bsToADYears, 'years').subtract(bsToADMonths, 'months').subtract(bsToADDays, 'days').format('YYYY-MM-DD');
                            end = moment(end, 'YYYY-MM-DD').subtract(bsToADYears, 'years').subtract(bsToADMonths, 'months').subtract(bsToADDays, 'days').format('YYYY-MM-DD');
                        }
    ...
    if ($('#product_pr_date_filter').length == 1) {
        $('#product_pr_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
            var yearFormat = $('#year-format').val(); // Assuming there's a mechanism to select year format
            if (yearFormat === 'BS') {
                start = convertToAD(start.format('YYYY-MM-DD'));
                end = convertToAD(end.format('YYYY-MM-DD'));
            }
    ...
    product_purchase_report = $('table#product_purchase_report_table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                var dateFilterVal = $('#product_pr_date_filter').val();
                if (dateFilterVal) {
                    var dates = dateFilterVal.split(' ~ ');
                    var yearFormat = $('#year-format').val(); // Assuming there's a way to select the year format
                    if (yearFormat === 'BS') {
                        d.start_date = convertToAD(dates[0]);
                        d.end_date = convertToAD(dates[1]);
                    } else {
    ...
    if ($('#ppr_date_filter').length == 1) {
        $('#ppr_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
            var yearFormat = $('#year-format').val(); // Example: Get the calendar system (BS/AD)
            if (yearFormat === 'BS') {
                start = convertToAD(start.format('YYYY-MM-DD'));
                end = convertToAD(end.format('YYYY-MM-DD'));
            }
            $('#ppr_date_filter span').html(start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format));
    ...
    sell_payment_report = $('table#sell_payment_report_table').DataTable({
        ...
        ajax: {
            url: '/reports/sell-payment-report',
            data: function(d) {
                var dateFilterVal = $('input#spr_date_filter').val();
                if (dateFilterVal) {
                    var dates = dateFilterVal.split(' ~ ');
                    var yearFormat = $('#year-format').val(); // Assuming you have a way to select the year format
                    if (yearFormat === 'BS') {
    ...
    if ($('#ir_purchase_date_filter').length == 1) {
        $('#ir_purchase_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
            ...
            $('#ir_purchase_date_filter').val(start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format));
            items_report_table.ajax.reload();
        });
    ...
    if ($('#ir_sale_date_filter').length == 1) {
        $('#ir_sale_date_filter').daterangepicker(dateRangeSettings, function(start, end) {
            ...
            $('#ir_sale_date_filter').val(start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format));
            items_report_table.ajax.reload();
        });
    ...
    items_report_table = $('#items_report_table').DataTable({
        ...
        ajax: {
            ...
            data: function(d) {
                var purchaseDate = $('#ir_purchase_date_filter').val().split(' ~ ');
                var saleDate = $('#ir_sale_date_filter').val().split(' ~ ');
                
                if (purchaseDate.length === 2) {
                    d.purchase_start = purchaseDate[0];
                    d.purchase_end = purchaseDate[1];
                }
        
                if (saleDate.length === 2) {
                    d.sale_start = saleDate[0];
                    d.sale_end = saleDate[1];
                }
        
                // Include other parameters
                d.supplier_id = $('#ir_supplier_id').val();
                d.customer_id = $('#ir_customer_id').val();
                d.location_id = $('#ir_location_id').val();
            }
        },
        columns: [
            { data: 'product_name', name: 'p.name' },
...
function updatePurchaseSell() {
    start = $('#purchase_sell_date_filter')
            .data('daterangepicker')
            .startDate.format('YYYY-MM-DD');
        // Check the current year format (assuming there's an element to hold the year format)
        var yearFormat = $('#year-format').val(); // Ensure this element's value is set correctly elsewhere in your code
        if (yearFormat === 'BS') {
            start = convertToAD(start);
        }
...
function updateStockAdjustmentReport() {
    ...
    // Assuming you have a way to select the year format, e.g., a select box with id="year-format"
    var yearFormat = $('#year-format').val(); // This line is new
    if (yearFormat === 'BS') {
        start = convertToAD(start);
        end = convertToAD(end);
    }
