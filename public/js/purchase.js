...
if (yearFormat == 'BS') {
    //Datetime picker for BS (Nepali) date
    $('#transaction_date').datetimepicker({
        format: 'MM/DD/YYYY HH:mm', // Set the desired format
        ignoreReadonly: true,
        locale: 'en', // Set the locale to English or the default language
    });

    // Get the initial selected date
    var selectedDate = $('#transaction_date').data('date');

    // Convert the initial selected date to Nepali
    var nepaliDate = moment(convertToNepali(selectedDate)); // Convert to Moment.js object

    // Set the selected date on the datetimepicker
    $('#transaction_date').data('date', nepaliDate); // Set the Nepali date directly

    // Update the input field's value to reflect the Nepali date
    $('#transaction_date').val(nepaliDate.format('MM/DD/YYYY HH:mm'));

    // Convert the Nepali date to AD format
    var addate = moment(convertToAD(nepaliDate)); 

    // Update the hidden input field 'transaction_date_ad' with the AD date
    $('#transaction_date_ad').val(addate.format('YYYY-MM-DD HH:mm:ss'));
} else {
    //Datetime picker for AD (Gregorian) date
    $('#transaction_date').datetimepicker({
        format: 'MM/DD/YYYY HH:mm', // Set the desired format
        ignoreReadonly: true,
        locale: 'en', // Set the locale to English or the default language
    ...
    // Convert the initial selected date to Moment.js object
    var momentSelectedDate = moment(selectedDate); 

    // Set the selected date on the datetimepicker
    $('#transaction_date').data('date', momentSelectedDate); // Set the AD date directly

    // Update the input field's value to reflect the selected date
    $('#transaction_date').val(momentSelectedDate.format('MM/DD/YYYY HH:mm'));

    // Convert the selected date to AD format (already in AD)
    var addate = moment(momentSelectedDate);
    ...
    purchase_table = $('#purchase_table').DataTable({
        ...
        ajax: {
            ...
            var yearFormat = document.getElementById('year-format').value; // Fetch the current year format
            if ($('#purchase_list_filter_date_range').val()) {
                var start = $('input#purchase_list_filter_date_range')
                    .data('daterangepicker')
                    .startDate.format('YYYY-MM-DD');
                var end = $('input#purchase_list_filter_date_range')
                    .data('daterangepicker')
                    .endDate.format('YYYY-MM-DD');

                // Convert dates if the year format is BS
                if (yearFormat === 'BS') {
            ...
            // Assign the (possibly converted) dates
            d.start_date = start;
            d.end_date = end;
