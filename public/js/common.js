...
// Get the session data from the input element
var yearFormat =document.getElementById('year-format').value;

// Check if yearFormat is not empty
if (yearFormat !== '') {
    // Use the yearFormat variable in your JavaScript logic
    console.log('Year Format:', yearFormat);
} else {
    // Handle the case when yearFormat is empty or not available
    console.log('Year Format is not available');
}
...
if(yearFormat == "BS")
{
    function convertToNepali(date) {
        ...
        momentDate.add(bsToADYears, 'years');
        momentDate.add(bsToADMonths, 'months');
        momentDate.add(bsToADDays, 'days');
    ...
    function convertToAD(nepaliDate) {
        ...
        // Use moment.js to manipulate the date
        const momentDate = moment(nepaliDate);
    ...
    // Convert Gregorian dates to Nepali BS format
    var nepaliStartDate = convertToNepali(startDate);
    var nepaliEndDate = convertToNepali(endDate);

    
    
        // Convert Gregorian dates to Nepali BS format
        var customstart = convertToNepali(financial_year.start);
        var customend = convertToNepali(financial_year.end);

        // Assuming convertToNepali is defined elsewhere and correctly converts Gregorian dates to Nepali BS dates

Object.keys(ranges).forEach(key => {
    // Convert both start and end dates for each range
    ranges[key] = ranges[key].map(date => convertToNepali(date));
});
    ...
    var dateRangeSettings = {
        ...
        locale: {
            ...
            toLabel: '~',
            monthNames: [
                'Baishakh', // Replace these placeholders with actual Nepali month names
                'Jestha',
                'Ashadh',
                'Shrawan',
                'Bhadra',
...
$(document).ready(function() {
    ...
    function convertToNepalis(date) {
        const bsToADYears = 56;
        const bsToADMonths = 8;
        const bsToADDays = 17;
    ...
    var nepaliDate = convertToNepalis(formattedDate);
    }
    else{
        nepaliDate = formattedDate;
    }
    // Log only the date part
    console.log(nepaliDate.format('YYYY-MM-DD HH:mm:ss'));
    
    $('#current-date-home').text(nepaliDate.format('YYYY-MM-DD HH:mm:ss'));
