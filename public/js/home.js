$(document).ready(function() {
    ...
    if ($('#dashboard_date_filter').length == 1) {
        $('#dashboard_date_filter').daterangepicker(customdateRangeSetting, function(starts, ends) {
            console.log(starts.format('YYYY-MM-DD'))
            console.log(ends.format('YYYY-MM-DD'))
            var yearFormat = $('#year-format').val(); // Assuming you have a way to select the year format
            if (yearFormat === 'BS') {
                start = convertToAD(starts);
                end = convertToAD(ends);
            } else {
                start = starts
                end = ends
            }
            ...
            $('#dashboard_date_filter span').html(
                starts.format(moment_date_format) + ' ~ ' + ends.format(moment_date_format)
            );
    ...
    $(`#dashboard_business_location_filter`).click(function(e){
        console.log(31371231278)
    })
...
function update_statistics(start, end) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('.total_invoice').html(__currency_trans_from_en(data.total_invoice, true));
