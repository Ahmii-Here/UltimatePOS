...
class BusinessUtil extends Util
{
    ...
    public function getCurrentFinancialYear($business_id)
    {
        ...
        $fy_start_date = $business->fy_start_date;
        $year_format = $business->year_format;
        $fy_end_date = $business->fy_start_date - 1;
        ...
        $businessLocations = BusinessLocation::where('business_id', $business_id)->first();

        if ($year_format === 'AD') {
            $fy_end_date = $end_year % 4 === 0 ? $fy_start_date : $fy_start_date - 1;
        }

        $start_date = $start_year . '-' . str_pad($start_month, 2, 0, STR_PAD_LEFT) . "-" . $fy_start_date;
        $end_date = $end_year . '-' . str_pad($end_month, 2, 0, STR_PAD_LEFT) . "-" . $fy_end_date;
