...
class Transaction extends Model
{
    ...
    public function getTransactionDateAttribute($value)
    {
       ...
       $yearFormat = Business::where('id', request()->session()->get('user.business_id'))->value('year_format');
        ...
        $timezone = Carbon::parse($value)->format('H:i');
        if ($yearFormat == "BS") {
            return NepaliCalendar::AD2BS(Carbon::parse($value)->format('Y-m-d'), [
                'lang' => 'en',
                'return_type' => 'string'
            ]) . ' ' . $timezone;

        } else {
            return $value; // Return only the timezone if the condition is not 1
        }
