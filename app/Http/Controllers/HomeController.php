...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $users = User::where('business_id', $business_id)->count();
        $customers = Contact::where('business_id', $business_id)->where('type', 'LIKE', 'customer')->count();
        $products = Product::where('business_id', $business_id)->count();
        $suppliers = Contact::where('business_id', $business_id)->where('type', 'LIKE', 'supplier')->count();
        $overall_tax = 0;

        return view('home.index', compact('sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'common_settings', 'is_admin', 'users', 'customers', 'suppliers', 'products', 'overall_tax'));
    }
