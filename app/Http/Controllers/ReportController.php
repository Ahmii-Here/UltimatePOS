...
class ReportController extends Controller
{
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $products_var = $this->productUtil->getProductvar($business_id, $filters, $for);
            ...
            $datatable = Datatables::of($products_var)
                ...
                ->addColumn('action', function ($row) {
                    return '<a class="btn btn-info btn-xs col-sm-12" href="' . action([\App\Http\Controllers\ProductController::class, 'productStockHistory'], [$row->product_id]) .
                        '?location_id=' . $row->location_id . '&variation_id=' . $row->variation_id .
                        '"><i class="fas fa-history"></i> ' . __('lang_v1.product_stock_history') . '</a> <button class="col-sm-12 btn btn-warning btn-xs view-details" data-href="' . action([\App\Http\Controllers\ProductController::class, 'view'], [$row->product_id]) . '">View Details</button>';
                })
                ->addColumn('variation', function ($row) {
                    $variation = '';
                    if ($row->type == 'variable') {
                        $variation .= $row->product_variation ;
                    }
                ...
                ->setRowAttr([
                    'data-href' => function ($row) {
                        // Return the URL for fetching variation details, adjust according to actual routing
                        return action([\App\Http\Controllers\ProductController::class, 'view'], [$row->product_id]);
                    },
                ])
        ...
        if ($request->ajax()) {
            ...
            $datatable->editColumn(
                ...
                ->editColumn('transaction_date', function ($row) {
                    ...
                    $yearFormat = Business::where('id', request()->session()->get('user.business_id'))->value('year_format');
                    $timezone = Carbon::parse($row->transaction_date)->format('H:i');
                    if ($yearFormat == "BS") {
                    return NepaliCalendar::AD2BS(Carbon::parse($row->transaction_date)->format('Y-m-d'), [
                        'lang' => 'en',
                        'return_type' => 'string'
                    ]) . ' ' . $timezone;
                    }
    ...
    public function getProfit($by = null)
    {
        ...
        if ($by == 'date') {
            $datatable->editColumn('transaction_date', function ($row) {
                // '{{@format_datetime($transaction_date)}}'
                $yearFormat = Business::where('id', request()->session()->get('user.business_id'))->value('year_format');
                $timezone = Carbon::parse($row->transaction_date)->format('H:i');
                if ($yearFormat == "BS") {
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('purchase_date', function ($row) {
                    ...
                    $yearFormat = Business::where('id', request()->session()->get('user.business_id'))->value('year_format');
                    $timezone = Carbon::parse($row->purchase_date)->format('H:i');
                    if ($yearFormat == "BS") {
                    return NepaliCalendar::AD2BS(Carbon::parse($row->purchase_date)->format('Y-m-d'), [
                        'lang' => 'en',
                        'return_type' => 'string'
                    ]) . ' ' . $timezone;
                    }
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($activities)
                ->editColumn('created_at', function ($row) {
                    ...
                    $yearFormat = Business::where('id', request()->session()->get('user.business_id'))->value('year_format');
                    $timezone = Carbon::parse($row->created_at)->format('H:i');
                    if ($yearFormat == "BS") {
                    return NepaliCalendar::AD2BS(Carbon::parse($row->created_at)->format('Y-m-d'), [
                        'lang' => 'en',
                        'return_type' => 'string'
                    ]) . ' ' . $timezone;
                    }
